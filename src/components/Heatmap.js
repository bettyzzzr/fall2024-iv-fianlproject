...
const Heatmap = ({ data }) => {
  ...
  useEffect(() => {
    ...
    // Dynamic color scale based on selected metric
    const color = d3.scaleSequential(
      metric === "Population"
        ? d3.interpolateGreens // Use green for Population
        : d3.interpolateBlues // Use blue for GDP
    ).domain(d3.extent(data, (d) => +d[metric])); // Adjust domain based on the selected metric
